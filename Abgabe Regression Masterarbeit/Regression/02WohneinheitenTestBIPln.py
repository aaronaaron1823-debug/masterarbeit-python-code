...
FEATURE_NAMES_EXT = ["const", "lnGpc", "U", "E", "lnA", "Hsize"]
...
def make_design(df_sub):
    ...
    Regressoren: lnGpc, U, E (Niveau), lnA, Haushaltsgröße (Hsize).
    """
    y, expo, lnGpc, U, E, Xk = base(df_sub)

    A = df_sub[ACOL].to_numpy(float)         
    H = df_sub[HCOL].to_numpy(float)         
    lnA = safe_log(A)                        

    X = sm.add_constant(np.column_stack([lnGpc, U, E, lnA, H]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: lnGpc, U, E, lnA(Fläche), Haushaltsgröße) ---")
...
wald_print(nb_boot, R_joint, "Gemeinsam lnGpc=U=E=lnA=Hsize=0")
wald_print(nb_boot, R_lngpc, "Einzeln lnGpc=0 (log BIP je Kopf)")
wald_print(nb_boot, R_U,     "Einzeln U=0")
wald_print(nb_boot, R_E,     "Einzeln E=0 (Strom, Niveau)")
