...
SHOW_FIG = True       
FIG_DIR  = "figures"
FIG_NAME = "belegte_wohneinheiten_nb2_bootstrap.pdf"

# 
FEATURE_NAMES_EXT = ["const", "U", "lnA", "Hsize"]
...
#
cols_used = [XCOL, YCOL, UCOL, ACOL, HCOL]
...
def make_design(df_sub):
    """
    Designmatrix:
    Regressoren: U, lnA (Fläche), Hsize.
    """
    y, expo, U, Xk = base(df_sub)
    A = df_sub[ACOL].to_numpy(float)         
    H = df_sub[HCOL].to_numpy(float)         
    lnA = safe_log(A)

    X = sm.add_constant(np.column_stack([U, lnA, H]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: U, lnA(Fläche), Haushaltsgröße) ---")
...
R_joint[0, name_to_idx_ext["U"]]     = 1.0
R_joint[1, name_to_idx_ext["lnA"]]   = 1.0
R_joint[2, name_to_idx_ext["Hsize"]] = 1.0

wald_print(nb_boot, R_joint, "Gemeinsam U=lnA=Hsize=0")
...
# 
plt.scatter(Xk_tr, y_tr,    s=18, label="Training: beobachtet", color="tab:blue")
plt.scatter(
    Xk_tr, yhat_tr, s=18,
    facecolors="none", edgecolors="tab:blue",
    label="Training: vorhergesagt"
)

# 
if has_val:
    plt.scatter(Xk_val, y_val, s=18, label="Validation: beobachtet", color="black")
    plt.scatter(
        Xk_val, yhat_val, s=18,
        facecolors="none", edgecolors="black",
        label="Validation: vorhergesagt"
    )
