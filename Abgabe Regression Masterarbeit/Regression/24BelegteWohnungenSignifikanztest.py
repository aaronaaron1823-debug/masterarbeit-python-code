...
# 
FILE  = "251008_MA_18_belegte Wohneinheiten.xlsx"
XCOL  = "Bevölkerung"        
YCOL  = "belegte Wohnungen"
GCOL  = "BIP (mrd.)"
UCOL  = "Urbanisierung"      
ACOL  = "Fläche qkm"         
HCOL  = "Haushalt"           
...
# 
FEATURE_NAMES_EXT = ["const", "lnBIPpk", "U", "lnA", "Hsize"]
...
# 
cols_used = [XCOL, YCOL, GCOL, UCOL, ACOL, HCOL]
...
def base(df_sub):
    """
    Erzeugt Grundgrößen für das Modell.
    Gibt zurück: Zielvariable Y, Exposure (Bevölkerung),
    ln(BIP je Kopf), U sowie Xk (Bevölkerung).
    """
    ...
    lnBIPpk = safe_log(GDPpc)                      

    expo  = pop                                    
    return Y, expo, lnBIPpk, U, Xk
...
def make_design(df_sub):
    ...
    Regressoren: ln(BIPpk), U, lnA (Fläche), Hsize.
    """
    y, expo, lnBIPpk, U, Xk = base(df_sub)
    A = df_sub[ACOL].to_numpy(float)         
    H = df_sub[HCOL].to_numpy(float)         
    lnA = safe_log(A)                        

    X = sm.add_constant(np.column_stack([lnBIPpk, U, lnA, H]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: ln(BIPpk), U, lnA(Fläche), Haushaltsgröße) ---")
...
R_lnBIPpk = np.zeros((1, len(FEATURE_NAMES_EXT))); R_lnBIPpk[0, name_to_idx_ext["lnBIPpk"]] = 1.0
...
R_joint = np.zeros((4, len(FEATURE_NAMES_EXT)))
R_joint[0, name_to_idx_ext["lnBIPpk"]] = 1.0
R_joint[1, name_to_idx_ext["U"]]       = 1.0
R_joint[2, name_to_idx_ext["lnA"]]     = 1.0
R_joint[3, name_to_idx_ext["Hsize"]]   = 1.0

wald_print(nb_boot, R_joint,  "Gemeinsam lnBIPpk=U=lnA=Hsize=0")
wald_print(nb_boot, R_lnBIPpk,"Einzeln lnBIPpk=0")
...
# 
print("\n=== [Modellvergleich: jeweils ein Regressor weggelassen] ===")

# 
if len(df_val) > 0:
    y_val_lor, expo_val_lor, X_val_lor, Xk_val_lor = make_design(df_val)
else:
    y_val_lor = expo_val_lor = X_val_lor = Xk_val_lor = None

full_features = FEATURE_NAMES_EXT[:]         
regressors   = [f for f in full_features if f != "const"]

lor_summary = []

for drop in regressors:
    keep = [f for f in full_features if f != drop]
    idxs = [full_features.index(f) for f in keep]

    
    X_tr_sub = X_tr[:, idxs]

    
    nb_sub, alpha_sub = fit_nb2_with_alpha(X_tr_sub, y_tr_s, expo_tr)
    r2_mcf_sub, ll_sub, ll0_sub, aic_sub = mcfadden_r2(
        nb_sub, X_tr_sub, y_tr_s, expo_tr, alpha_sub
    )

    
    yhat_tr_sub = nb_sub.fittedvalues * SCALE
    metr_tr_sub = reg_metrics(y_tr, yhat_tr_sub)

    
    if len(df_val) > 0:
        X_val_sub = X_val_lor[:, idxs]
        yhat_val_s_sub = nb_sub.predict(X_val_sub, exposure=expo_val_lor)
        yhat_val_sub   = yhat_val_s_sub * SCALE
        metr_val_sub   = reg_metrics(y_val_lor, yhat_val_sub)
        val_mape = metr_val_sub[3]
        val_r2   = metr_val_sub[5]
    else:
        val_mape = np.nan
        val_r2   = np.nan
    ...
    lor_summary.append({
        ...
        "alpha":        alpha_sub,
        "AIC":          aic_sub,
        "McFadden_R2":  r2_mcf_sub,
        "Train_MAPE":   metr_tr_sub[3],
        "Train_R2":     metr_tr_sub[5],
        "Val_MAPE":     val_mape,
        "Val_R2":       val_r2,
    })

lor_df = pd.DataFrame(lor_summary)
print(lor_df.to_string(index=False, float_format=lambda x: f"{x:,.4g}"))
