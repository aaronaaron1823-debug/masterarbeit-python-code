...
# 
FILE  = "251008_MA_18_Buro.xlsx"
XCOL  = "Bevölkerung"                 
YCOL  = "Büroimmobilien Anzahl"
GCOL  = "BIP (mrd.)"                  
UCOL  = "Urbanisierung"               
ECOL  = "Strom (mrd.)"                
ACOL  = "Fläche qkm"                  
HCOL  = "Bürofläche pro Person"       
SCOL  = "Status"
...
def base(df_sub):
    """
    - Y: Bürogebäude-Anzahl
    - expo: Offset (Bevölkerung)
    - U: Urbanisierungsgrad (0..1)
    - Xk: Bevölkerung (für Plot)
    """
...
def make_design(df_sub):
    ...
    X = sm.add_constant(U.reshape(-1, 1), has_constant="add")
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — finales Büro-Modell (Offset log(Bev.), Regressor: U) ---")
...
print_reg_metrics("Train NB2 (U)", reg_metrics(y_tr, yhat_tr))


# 
print("\n=== [Wald-Test auf Signifikanz (U)] ===")
R_U = np.zeros((1, len(FEATURE_NAMES_EXT)))
R_U[0, FEATURE_NAMES_EXT.index("U")] = 1.0
...
plt.xlabel(XCOL)
plt.ylabel(YCOL)
plt.title("NB2-GLM Bürogebäude (Bootstrap) — Observed vs Predicted (Training, Regressor: U)")
...
if len(df_val) > 0:
    ...
    print_reg_metrics("Val NB2 (U)", reg_metrics(y_val, yhat_val))
    ...
    plt.xlabel(XCOL)
    plt.ylabel(YCOL)
    plt.title("NB2-GLM Bürogebäude (Bootstrap) — Observed vs Predicted (Validation, Regressor: U)")
    plt.legend(fontsize=9)
    plt.tight_layout()
    plt.show()
else:
