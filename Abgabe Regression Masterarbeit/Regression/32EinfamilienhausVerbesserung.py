...
# 
FEATURE_NAMES_EXT = ["const", "BIPpk", "U", "E"]
...
# 
cols_used = [XCOL, YCOL, GCOL, UCOL, ECOL]
...
def make_design(df_sub):
    """
    Designmatrix:
    Basisregressoren: BIPpk, U, E (ohne Fläche, ohne Hsize).
    """
    y, expo, BIPpk, U, E, Xk = base(df_sub)

    X = sm.add_constant(np.column_stack([BIPpk, U, E]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: BIPpk, U, E) ---")
...
wald_print(nb_boot, R_joint, "Gemeinsam BIPpk=U=E=0")
