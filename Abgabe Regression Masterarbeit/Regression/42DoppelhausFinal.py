...
# 
FILE  = "251008_MA_18_Doppelhaus.xlsx"
XCOL  = "Bevölkerung"                
YCOL  = "Doppelhaushälfte Anzahl"    
ECOL  = "Strom (mrd.)"               
SCOL  = "Status"                     
...
def base(df_sub):
    """
    - Y: Zielvariable (Doppelhaushälften)
...
R_E = np.zeros((1, len(FEATURE_NAMES_EXT))); R_E[0, FEATURE_NAMES_EXT.index("E")] = 1.0
wald_print(nb_boot, R_E, "Einzeln E=0 (Strom)")


# 
print("\n=== [Vergleich mit Nullmodell (nur Offset & Intercept)] ===")
X_null = sm.add_constant(np.zeros((X_tr.shape[0], 0)))
...
plt.xlabel(XCOL)
plt.ylabel(YCOL)
plt.title("NB2-GLM Doppelhaus (Bootstrap) — Observed vs Predicted (Training, nur Strom)")
...
if len(df_val) > 0:
    ...
    plt.xlabel(XCOL)
    plt.ylabel(YCOL)
    plt.title("NB2-GLM Doppelhaus (Bootstrap) — Observed vs Predicted (Validation, nur Strom)")
    plt.legend(fontsize=9)
    plt.tight_layout()
    plt.show()
else:
