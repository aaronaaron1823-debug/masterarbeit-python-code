...
# 
FEATURE_NAMES_EXT = ["const", "U", "E", "lnA", "Hsize"]
...
def make_design(df_sub):
    ...
    Regressoren: U, E (Niveau), lnA (Fläche), Hsize.
    """
    y, expo, U, E, Xk = base(df_sub)
    A = df_sub[ACOL].to_numpy(float)         
    H = df_sub[HCOL].to_numpy(float)         
    lnA = safe_log(A)                        

    X = sm.add_constant(np.column_stack([U, E, lnA, H]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: U, E, lnA(Fläche), Haushaltsgröße) ---")
...
wald_print(nb_boot, R_joint, "Gemeinsam U=E=lnA=Hsize=0")
