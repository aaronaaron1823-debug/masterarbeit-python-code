...
SHOW_FIG = False         
FIG_DIR  = "figures"
FIG_NAME = "wohnimmobilien_lnBIP_nb2_bootstrap.pdf"

# 
FEATURE_NAMES_EXT = ["const", "lnBIP"]
...
def base(df_sub):
    ...
    lnBIP = safe_log(GDPpc)

    expo = pop
    return Y, expo, lnBIP, Xk


# 
def make_design(df_sub):
    y, expo, lnBIP, Xk = base(df_sub)
    X = sm.add_constant(lnBIP.reshape(-1, 1))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressor: lnBIP je Kopf) ---")
...
R_lnBIP = np.zeros((1, len(FEATURE_NAMES_EXT)))
R_lnBIP[0, name_to_idx_ext["lnBIP"]] = 1.0

wald_print(nb_boot, R_lnBIP, "Einzeln lnBIP=0")
