...
# 
FEATURE_NAMES_EXT = ["const", "E", "lnA", "Hsize"]
...
def make_design(df_sub):
    ...
    Regressoren: E (Niveau), lnA (Fläche), Hsize.
    """
    ...
    X = sm.add_constant(np.column_stack([E, lnA, H]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: E, lnA(Fläche), Haushaltsgröße) ---")
...
R_joint[0, name_to_idx_ext["E"]]     = 1.0
R_joint[1, name_to_idx_ext["lnA"]]   = 1.0
R_joint[2, name_to_idx_ext["Hsize"]] = 1.0

wald_print(nb_boot, R_joint, "Gemeinsam E=lnA=Hsize=0")
