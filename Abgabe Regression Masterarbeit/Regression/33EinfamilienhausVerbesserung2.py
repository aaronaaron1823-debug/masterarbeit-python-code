...
# 
FILE  = "251008_MA_18_Einfamilienhauser.xlsx"
XCOL  = "Bevölkerung"                 
YCOL  = "Einfamilienhauser Anzahl"    
GCOL  = "BIP (mrd.)"
ECOL  = "Strom (mrd.)"                
SCOL  = "Status"                      
...
# 
FEATURE_NAMES_EXT = ["const", "BIPpk", "E"]
...
def base(df_sub):
    """
    Grundgrößen:
    - Y: Zielvariable (Einfamilienhäuser)
    - expo: Offset (Bevölkerung)
    - BIPpk: BIP je Kopf (Niveau)
    - E: Strom (Niveau)
    ...
    expo = pop                               
    return Y, expo, BIPpk, E, Xk


def make_design(df_sub):
    """
    Designmatrix:
    Basisregressoren: BIPpk, E (ohne U, ohne Fläche, ohne Hsize).
    """
    y, expo, BIPpk, E, Xk = base(df_sub)

    X = sm.add_constant(np.column_stack([BIPpk, E]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: BIPpk, E) ---")
...
R_joint = np.zeros((2, len(FEATURE_NAMES_EXT)))
R_joint[0, name_to_idx_ext["BIPpk"]] = 1.0
R_joint[1, name_to_idx_ext["E"]]     = 1.0

wald_print(nb_boot, R_joint, "Gemeinsam BIPpk=E=0")
...
plt.xlabel(XCOL)
plt.ylabel(YCOL)
plt.title("NB2-GLM EFH (Bootstrap) — Observed vs Predicted (Training)")
...
if len(df_val) > 0:
    ...
    plt.xlabel(XCOL)
    plt.ylabel(YCOL)
    plt.title("NB2-GLM EFH (Bootstrap) — Observed vs Predicted (Validation)")
    plt.legend(fontsize=9)
    plt.tight_layout()
    plt.show()
else:
