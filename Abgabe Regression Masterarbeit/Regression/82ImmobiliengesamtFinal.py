...
def make_design_E(df_sub):
    """Designmatrix nur mit Stromverbrauch E als Regressor."""
    y, expo, reg_dict, Xk = base(df_sub)
    r = np.asarray(reg_dict["E"], float).reshape(-1, 1)
...
# 
print("\n" + "="*80)
print("=== NB2 (Bootstrap) — Ein-Regressor-Modell: E (Stromverbrauch) ===")

# 
y_tr, expo_tr, X_tr, Xk_tr = make_design_E(df_tr_boot)
y_tr_s = y_tr / SCALE
...
print("\n--- Metriken (Train) ---")
print_reg_metrics("Train NB2 (E)", metr_tr)

print("\n--- Wald-Test ---")
R = np.zeros((1, len(feat_names)))
R[0, 1] = 1.0
wald_print(nb_mod, R, "Einzeln E=0")

# 
if len(df_val) > 0:
    y_val, expo_val, X_val, Xk_val = make_design_E(df_val)
    yhat_val_s = nb_mod.predict(X_val, exposure=expo_val)
    yhat_val   = yhat_val_s * SCALE
    metr_val   = reg_metrics(y_val, yhat_val)

    print("\n--- Metriken (Validation) ---")
    print_reg_metrics("Val NB2 (E)", metr_val)
    ...
    plt.scatter(
        ...
        label="NB2 ŷ (Validation, E)"
    )
    plt.xlabel(XCOL)
    plt.ylabel(YCOL)
    plt.title("NB2-GLM (Bootstrap) — Observed vs Predicted (Validation, nur E)")
...
plt.scatter(
    Xk_tr, yhat_tr, s=18,
    facecolors="none", edgecolors="tab:blue",
    label="NB2 ŷ (Train, Boot, E)"
)
plt.xlabel(XCOL)
plt.ylabel(YCOL)
plt.title("NB2-GLM (Bootstrap) — Observed vs Predicted (Training, nur E)")
plt.legend(fontsize=9)
plt.tight_layout()
plt.show()
