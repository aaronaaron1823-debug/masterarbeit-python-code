...
# 
FEATURE_NAMES_EXT = ["const", "lnBIPpk", "U", "E", "lnA", "Hsize"]
...
def base(df_sub):
    ...
    ln(BIP je Kopf), U, E sowie Xk (Bevölkerung).
    """
...
def make_design(df_sub):
    ...
    Regressoren: ln(BIPpk), U, E (Niveau), lnA (Fläche), Hsize.
    """
    y, expo, lnBIPpk, U, E, Xk = base(df_sub)
    A = df_sub[ACOL].to_numpy(float)         
    H = df_sub[HCOL].to_numpy(float)         
    lnA = safe_log(A)                        

    X = sm.add_constant(np.column_stack([lnBIPpk, U, E, lnA, H]))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressoren: ln(BIPpk), U, E, lnA(Fläche), Haushaltsgröße) ---")
...
wald_print(nb_boot, R_joint,  "Gemeinsam lnBIPpk=U=E=lnA=Hsize=0")
