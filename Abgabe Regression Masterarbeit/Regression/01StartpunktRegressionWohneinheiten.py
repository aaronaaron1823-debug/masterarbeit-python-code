...
# 
FEATURE_NAMES_RED = ["const", "lnGpc", "A"] 
...
cols_used = [XCOL, YCOL, GCOL, UCOL, ECOL, ACOL]
...
def base(df_sub):
    ...
    expo  = pop                                    
    return Y, expo, lnGpc, U, lnE, Xk


#
def make_design_red(df_sub):
    """
    Erzeugt Zielvariable, Exposure und Designmatrix X für das reduzierte NB2-Modell.
    Regressoren: lnGpc, Fläche (A).
    """
    y, expo, lnGpc, U, lnE, Xk = base(df_sub) 
    A = df_sub[ACOL].to_numpy(float)           

    X = sm.add_constant(np.column_stack([lnGpc, A]))
    return y, expo, X, Xk
...
def bootstrap_df(df_sub, n_samples=1000, random_state=42):
    """
    Bootstrap-Resampling mit Zurücklegen, um n_samples zu erzeugen.
    """
...
# 
y_tr, expo_tr, X_tr, Xk_tr = make_design_red(df_tr_boot)
y_tr_s = y_tr / SCALE

nb_red, alpha_hat_red = fit_nb2_with_alpha(X_tr, y_tr_s, expo_tr)

print("\n--- NB2 (Bootstrap, reduziert) — Formel (Offset log(Bev.), Regressoren: lnGpc, Fläche) ---")
print(f"alpha (Dispersion) = {alpha_hat_red:.6g}")
print(coef_table(nb_red, FEATURE_NAMES_RED).to_string(float_format=lambda x: f"{x:,.6g}"))

# 
r2_mcf, ll, ll0, aic = mcfadden_r2(nb_red, X_tr, y_tr_s, expo_tr, alpha_hat_red)
print(f"LogLik={ll:,.3f} | LogLik(null)={ll0:,.3f} | AIC={aic:,.3f} | McFadden R^2={r2_mcf:.4f}")

# 
yhat_tr = nb_red.fittedvalues * SCALE

print("\n=== [NB2 (Bootstrap, reduziert) — Metriken (Train)] ===")
print_reg_metrics("Train NB2 Red Boot", reg_metrics(y_tr, yhat_tr))


# 
print("\n=== [Wald-Tests auf Signifikanz (NB2 Bootstrap, reduziert)] ===")

name_to_idx_red = {n: i for i, n in enumerate(FEATURE_NAMES_RED)}

# 
R_lngpc = np.zeros((1, len(FEATURE_NAMES_RED))); R_lngpc[0, name_to_idx_red["lnGpc"]] = 1.0
R_A     = np.zeros((1, len(FEATURE_NAMES_RED))); R_A[0,     name_to_idx_red["A"]]     = 1.0

# 
R_joint = np.zeros((2, len(FEATURE_NAMES_RED)))
R_joint[0, name_to_idx_red["lnGpc"]] = 1.0
R_joint[1, name_to_idx_red["A"]]     = 1.0

wald_print(nb_red, R_joint, "Gemeinsam lnGpc=A=0")
wald_print(nb_red, R_lngpc, "Einzeln lnGpc=0")
wald_print(nb_red, R_A,     "Einzeln Fläche=0")


# 
plt.figure(figsize=(10, 5))
plt.scatter(Xk_tr, y_tr,    s=18, label="Observed (Train, Boot)")
plt.scatter(Xk_tr, yhat_tr, s=18, facecolors="none", edgecolors="black",
            label="NB2 ŷ (Train, Boot, reduziert)")
plt.xlabel(XCOL)
plt.ylabel(YCOL)
plt.title("NB2-GLM (Bootstrap, reduziert) — Observed vs Predicted (Training)")
...
# 
if len(df_val) > 0:
    y_val, expo_val, X_val, Xk_val = make_design_red(df_val)

    #
    yhat_val_s = nb_red.predict(X_val, exposure=expo_val) 
    yhat_val   = yhat_val_s * SCALE

    print("\n=== [NB2 (Bootstrap, reduziert) — Metriken (Validation)] ===")
    print_reg_metrics("Val NB2 Red Boot", reg_metrics(y_val, yhat_val))

    #
    plt.figure(figsize=(10, 5))
    plt.scatter(Xk_val, y_val,    s=18, label="Observed (Validation)")
    plt.scatter(Xk_val, yhat_val, s=18, facecolors="none", edgecolors="black",
                label="NB2 ŷ (Validation, reduziert)")
    plt.xlabel(XCOL)
    plt.ylabel(YCOL)
    plt.title("NB2-GLM (Bootstrap, reduziert) — Observed vs Predicted (Validation)")
    plt.legend(fontsize=9)
    plt.tight_layout()
    plt.show()
else:
