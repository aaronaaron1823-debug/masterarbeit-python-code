...
# 
FILE  = "251008_MA_18_Mehrfamilienhaus.xlsx"
XCOL  = "Bevölkerung"                    
YCOL  = "Mehrfamilienhäuser Anzahl"
ECOL  = "Strom (mrd.)"                   
SCOL  = "Status"                         
...
plt.xlabel(XCOL)
plt.ylabel(YCOL)
plt.title("NB2-GLM MFH (Bootstrap) — Observed vs Predicted (Training, nur Strom)")
...
if len(df_val) > 0:
    ...
    plt.xlabel(XCOL)
    plt.ylabel(YCOL)
    plt.title("NB2-GLM MFH (Bootstrap) — Observed vs Predicted (Validation, nur Strom)")
    plt.legend(fontsize=9)
    plt.tight_layout()
    plt.show()
else:
