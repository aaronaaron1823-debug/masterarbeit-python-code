...
SHOW_FIG = False       
FIG_DIR  = "figures"
FIG_NAME = "efh_nb2_strom_bootstrap.pdf"
...
def make_design(df_sub):
    """
    Designmatrix:
    einziger Regressor: E (Strom).
    """
    y, expo, E, Xk = base(df_sub)
    X = sm.add_constant(E.reshape(-1, 1))
    return y, expo, X, Xk
...
print("\n--- NB2 (Bootstrap) — Formel (Offset log(Bev.), Regressor: E (Strom)) ---")
...
# 
print("\n=== [Vergleich mit Nullmodell (nur Offset & Intercept)] ===")
X_null = sm.add_constant(np.zeros((X_tr.shape[0], 0)))  # nur Intercept
nb_null, alpha_null = fit_nb2_with_alpha(X_null, y_tr_s, expo_tr)
r2_mcf_null, ll_null, ll0_null, aic_null = mcfadden_r2(
    nb_null, X_null, y_tr_s, expo_tr, alpha_null
)
print(f"Nullmodell AIC={aic_null:,.3f} | LogLik={ll_null:,.3f}")
